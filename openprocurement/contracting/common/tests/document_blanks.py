...
def not_found(self):
    ...
    response = self.app.post('/contracts/{}/documents?acc_token={}'.format(
        self.contract_id, self.contract_token), status=404, upload_files=[('invalid_name', 'name.doc', 'content')])
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
...
#    req.content_length = len(body)
    ...
    response = self.app.put('/contracts/{}/documents/{}?acc_token={}'.format(
        self.contract_id, doc_id, self.contract_token), status=404, upload_files=[
...
def patch_contract_document(self):
    ...
    response = self.app.patch_json('/contracts/{}/documents/{}?acc_token={}'.format(
        self.contract_id, doc_id, self.contract_token), {"data": {"documentType": None}})
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(doc_id, response.json["data"]["id"])
    self.assertNotIn("documentType", response.json["data"])
    ...
    response = self.app.patch_json('/contracts/{}/documents/{}?acc_token={}'.format(
        self.contract_id, doc_id, self.contract_token), {"data": {
            "description": "document description X", }}, status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.json['errors'], [{
        u'description': u"Can't update document in current (terminated) contract status",
        u'location': u'body', u'name': u'data'}])
...
def contract_change_document(self):
    ...
    response = self.app.patch_json('/contracts/{}/documents/{}?acc_token={}'.format(
        self.contract_id, doc_id, self.contract_token), {"data": {
            "documentOf": "change",
            "relatedItem": change['id'], }}, status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.json['errors'], [
...
def create_contract_document_json_invalid(self):
    response = self.app.post_json('/contracts/{}/documents?acc_token={}'.format(
        self.contract_id, self.contract_token), {'data': {
            'title': u'укр.doc',
            'url': self.generate_docservice_url(),
            'format': 'application/msword', }}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
